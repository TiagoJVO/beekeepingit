...
test("tapping Sign out actually starts sign-out (#836, FR-ONB, NFR-SEC-1)", async ({
  page,
}, testInfo) => {
  // TEMP #836 (exp branch only): prove the mechanism and that logout() runs.
  const R = `R${testInfo.repeatEachIndex}`;
  page.on("console", (m) => {
    const t = m.text();
    if (t.includes("[bk836]") && !t.includes("HEARTBEAT")) console.log(`${R} ${t}`);
  });
  ...
  const before = await signOut.boundingBox({ timeout: 2000 }).catch(() => null);
  console.log(
    `${R} BOX before scroll = ${JSON.stringify(before)} viewport=${JSON.stringify(page.viewportSize())}`,
  );
  await scrollFlutterViewTo(page, signOut);
  const after = await signOut.boundingBox({ timeout: 2000 }).catch(() => null);
  console.log(`${R} BOX after  scroll = ${JSON.stringify(after)}`);
  await signOut.click();
