...
bool hasFieldCode(
  List<ActivityAttributeError> errors,
  String field,
  String code,
) {
...
void main() {
  ...
  group('validateActivityAttributes: harvest', () {
    ...
    test(
      'missing honey_supers is rejected (required, primary yield metric)',
      () {
        final errors = validateActivityAttributes(activityTypeHarvest, {
          'honey_kg': 10,
        });
        expect(
          hasFieldCode(errors, 'attributes.honey_supers', 'required'),
          isTrue,
        );
      },
    );
    ...
    test('negative honey_supers is out of range', () {
      ...
      expect(
        hasFieldCode(errors, 'attributes.honey_supers', 'out_of_range'),
        isTrue,
      );
    });
  ...
  group('validateActivityAttributes: feeding', () {
    ...
    test('missing feed_amount is rejected', () {
      ...
      expect(
        hasFieldCode(errors, 'attributes.feed_amount', 'required'),
        isTrue,
      );
    });
  });
  ...
  group('validateActivityAttributes: treatment', () {
    ...
    test(
      'disease_specific without disease is rejected (conditional requirement)',
      () {
        final errors = validateActivityAttributes(activityTypeTreatment, {
          'treatment_context': treatmentContextDiseaseSpecific,
          'treatment_type': 'Timol',
    ...
    test(
      'detection_only without disease is rejected (conditional requirement)',
      () {
        final errors = validateActivityAttributes(activityTypeTreatment, {
    ...
    test('missing treatment_type when disease_specific is still rejected '
      ...
      expect(
        hasFieldCode(errors, 'attributes.treatment_type', 'required'),
        isTrue,
      );
    });
  ...
  group('validateActivityAttributes: generic', () {
    ...
    test('valid with notes', () {
      expect(
        validateActivityAttributes(activityTypeGeneric, {
          'notes': 'checked the entrance',
        }),
        isEmpty,
      );
    });
    ...
    test('rejects an attribute not part of the generic schema', () {
      final errors = validateActivityAttributes(activityTypeGeneric, {
        'honey_supers': 1,
      });
      expect(
        hasFieldCode(errors, 'attributes.honey_supers', 'invalid'),
