...
void main() {
  group('combineSyncStatus', () {
    ...
    test('re-emits when gateState changes independently of engine status', (
      () async {
    ...
    test('re-reads pendingCount on every re-emit, not just the first', () async {
      final engine = StreamController<EngineConnectivity>();
      addTearDown(engine.close);
      var callCount = 0;
    ...
    test('cancelling the combined stream cancels both source subscriptions', (
      () async {
        final engine = StreamController<EngineConnectivity>();
        final gate = StreamController<SyncGateState>();
        addTearDown(() {
