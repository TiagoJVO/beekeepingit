...
buildLoggedInContainer({required http.Client client}) async {
...
ProviderContainer _container(FakeAuthPlatform platform, http.Client client) {
...
void main() {
  ...
  group('code exchange (build())', () {
    test('success populates AuthSession and persists the refresh + id token', () async {
    ...
    test('refreshes when the access token is within 30s of expiry', () async {
      final client = MockClient((req) async {
        ...
        if (body['grant_type'] == 'refresh_token') {
          return _tokenResponse(req, access: 'refreshed-token', refresh: 'refresh-2');
        }
  ...
  group('refresh-token-rejected', () {
    test('a rejected refresh token clears the session without throwing', () async {
      final client = MockClient((req) async {
        final body = Uri(query: req.body).queryParameters;
        if (body['grant_type'] == 'refresh_token') {
          return http.Response(
  ...
  group('logout()', () {
    test('redirects to the front-channel end-session URL (id_token_hint) and clears local state', () async {
      final client = MockClient((req) async => _tokenResponse(req, idToken: 'id-1'));
      final (_, platform, notifier) = await buildLoggedInContainer(client: client);
      ...
      expect('${logoutUri.scheme}://${logoutUri.host}${logoutUri.path}', _endSessionUrl);
      expect(logoutUri.queryParameters['id_token_hint'], 'id-1');
      expect(logoutUri.queryParameters['post_logout_redirect_uri'], platform.redirectUri);

      expect(notifier.state.value, isNull);
      expect(platform.hasAnySession, isFalse);
    });

    test('degrades gracefully to locally-logged-out when discovery/redirect fails', () async {
    ...
    test('logging out with no session (already logged out) does not redirect', () async {
      final client = MockClient((req) async => _tokenResponse(req));
      final platform = FakeAuthPlatform();
      final container = _container(platform, client);
