...
void main() {
  ...
  // #649 (FR-ONB-1, FR-UX-1): a blocked save raises the required-name error,
  ...
  // #649's second criterion: a value that is still not valid (whitespace
  // only — the validator trims) keeps its error.
  ...
  testWidgets('drops a server field error once the user edits that field', (
    ...
    await tester.enterText(find.byKey(const Key('profile-name-field')), 'Ana');
    await tester.pumpAndSettle();
