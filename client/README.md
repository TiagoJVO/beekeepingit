...
| `OIDC_REGISTRATION_URL` | `https://auth.beekeepingit.local:8443/if/flow/beekeepingit-enrollment/` | Provider self-service **account-creation** entry point behind the login screen's "Create account" action (`#647`). Must be on the **same origin as `OIDC_ISSUER`** — the pending authorize request rides there in an origin-relative `next`. Empty hides the action |
...
| `lib/shell/`                | The persistent app shell (`FR-UX-2`, `#197`) — 5-tab bottom nav (apiaries · activities · **home** · journeys · todos, `#658`/`D-35`), header (contextual back, brand + screen title, sync-status pill, account), contextual honey FAB, offline banner. Every tab has a real screen; **Home is the only tab with no FAB** — its area is every area, so no single create action is the right one (`D-35`). Activities gained its own in `#634` ("Add activity" → `/activities/new`)                                                                                                                                                                    |
