...
/// #39 added a THIRD, [activitiesTable], and #45 a FOURTH/FIFTH,
/// [journeysTable]/[journeyPlanItemsTable] — each routed by
/// services/sync/api/coordinator.go's groupOpsByOwner to its OWN owning
/// service (activities, journeys — not apiaries) — getting this mapping
/// wrong would silently misroute an offline-created row to apiaries, which
/// doesn't own that table and would reject it). Any unrecognized table
/// defaults to the apiary entity type, preserving the previous hardcoded
/// behavior for safety; a genuinely new syncable table must add its own
/// mapping here alongside its schema entry (powersync_schema.dart).
