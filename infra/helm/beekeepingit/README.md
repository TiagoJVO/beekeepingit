...
supersedes for `authentik`/`minio`, and
[ADR-0016](../../../docs/adr/0016-replace-keycloak-with-authentik.md) for the Keycloak→Authentik
swap). A third shape — a chart of directly-vendored upstream
...
value in git), add a **thin local chart** here (`charts/authentik/`, `charts/minio/` are live
...
`beekeepingit-authentik-config`. That name is only stable because all three
...
`beekeepingit-dev-beekeepingit-authentik-config`; confirmed against the live cluster).
...
| Subchart        | What it is                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `postgres`      | PostgreSQL + PostGIS (D-6) via a CloudNativePG `Cluster` CR — schema-per-service, with TWO credentials per schema: `<schema>_migrator` (owns that schema's relations, used by the deploy-time migrate Job) and `<schema>_svc` (runtime, DML only). See [`../../README.md`](../../README.md) "Database roles" and ADR-0024                                                                                                                                                                                                                                                                                                                                                                 |
| `authentik`     | Generated config/Postgres Secrets + dev/CI-grade blueprint (provider/app/group/seed user) for OIDC IdP Authentik (D-7, ADR-0016), plus the BeekeepingIT branding: the brand entry's custom CSS and the `beekeepingit-authentik-email-templates` ConfigMap (#648). Sender is `email.fromName` + `email.fromAddress` (the address a placeholder until #417); template overrides load from `email.templateDir` (`/templates`), which the external release mounts at `/templates/email` on both the server and the worker (#858) — a cross-repo contract, so that value is pinned by `scripts/check-authentik-brand-posture.sh`. Authentik itself is a separate Flux `HelmRelease` (ADR-0012) |
