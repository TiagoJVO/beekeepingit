...
| `staging` | Scaleway Kapsule (D-26)      | `beekeepingit-staging` | `beekeepingit-rc.melargil.pt`, `auth.` / `admin.` prefixes (Cloudflare DNS, LE cert) | `tag: <release>` — moved by the promotion PR        | a `*-rc*` release's promotion PR merges                                      | `staging-gate` (ungated; secrets + `APP_HOST`/`AUTH_HOST`)  |
...
- `environments/<env>.yaml` is not what runs. #556: staging's gitops values never got
  ...
  renewal failed while the mirror looked right.
