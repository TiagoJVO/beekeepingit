...
func TestApiaryVerifier_BelongsToOrg_200IsTrue(t *testing.T) {
	...
	// auth.md §4) — journeys never asserts org membership to apiaries out of
	// band, it re-authenticates apiaries' own way.
...
// case this file exists for (mirrors activities'/apiaries' own #284 fix):
// apiaries' own GET /v1/apiaries/{id} 404s for an id that doesn't exist OR
// belongs to a different organization (ADR-0002 scope-hiding) — both must be
// treated as "reject this write", never as "allow it".
