...
// FR-HIS-1) — no DB/Docker dependency, mirrors apiaries/api/history_test.go
// and this package's own validate_test.go/types_test.go convention. The HTTP
// handler itself (getActivityHistory: tenancy/404, the real combined
// audit_log+sync_conflict_log query) is covered by the containerized suite
// in main_test.go, since it needs a real Postgres.
...
func TestTimelineRowToDTO_AuditRowWithActor(t *testing.T) {
	...
	change := []byte(`{"type":{"from":"harvest","to":"generic"}}`)
	...
	if got.EntityType != "activity" {
		t.Errorf("EntityType = %q, want activity", got.EntityType)
	}
	...
	if len(got.ChangedFields) != 1 || got.ChangedFields[0] != "type" {
		t.Errorf("ChangedFields = %v, want [type]", got.ChangedFields)
	}
	var gotChange map[string]any
	if err := json.Unmarshal(got.Change, &gotChange); err != nil {
		t.Fatalf("unmarshal Change: %v", err)
	}
	if _, ok := gotChange["type"]; !ok {
		t.Errorf("Change = %s, want a type field", got.Change)
	}
}
...
func TestTimelineRowToDTO_SupersededRowHasNoActorAndNoChangedFields(t *testing.T) {
	...
	// audit_log rows carry it, per ListEntityTimeline's own UNION shape), and
	// an unresolved/absent actor must come through as a nil pointer, never an
	// empty-string placeholder that could be confused with a real id.
	row := sqlcgen.ListEntityTimelineRow{
