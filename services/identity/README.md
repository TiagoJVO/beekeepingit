...
the `identity.users` table — the local projection of a Keycloak-authenticated
...
| `GET /v1/profile`                  | Keycloak JWT | Get (or lazily create, on first login) the caller's own profile. No org resolver — profile exists before any organization does.                             |
| `PATCH /v1/profile`                | Keycloak JWT | Partially update the caller's own profile (`name`/`email`/`locale`); 422 on invalid fields. Returns the full profile, including derived `profile_complete`. |
