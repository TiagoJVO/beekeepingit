...
// Seed inserts the dev/CI test user (devseed.KeycloakSub → devseed.UserID) so
...
func Seed(ctx context.Context, pool *pgxpool.Pool) error {
	_, err := pool.Exec(ctx, `
		...
		devseed.UserID, devseed.KeycloakSub, devseed.UserName, devseed.UserEmail, devseed.UserLocale)
	if err != nil {
