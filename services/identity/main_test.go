...
// TestIdentityService_ResolveByEmail covers the #468 internal
// GET /users/by-email/{email} endpoint: unauthenticated is rejected, the
// seeded user's email resolves (case-insensitively) to its user row, and an
// unknown email is a clean 404 rather than an internal error.
func TestIdentityService_ResolveByEmail(t *testing.T) {
	ctx := context.Background()
	...
	// Unauthenticated → 401.
	if rec := get("/internal/users/by-email/"+devseed.UserEmail, ""); rec.Code != http.StatusUnauthorized {
		t.Errorf("unauthenticated status = %d, want %d", rec.Code, http.StatusUnauthorized)
	}

	// Seeded email → 200 with the resolved user (case-insensitive match).
	rec := get("/internal/users/by-email/"+strings.ToUpper(devseed.UserEmail), "Bearer "+token)
	if rec.Code != http.StatusOK {
		t.Fatalf("resolve by email status = %d, want %d, body = %s", rec.Code, http.StatusOK, rec.Body.String())
	}
	var got api.UserResponse
	if err := json.Unmarshal(rec.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode: %v", err)
	...
	// Unknown email → 404, not an internal error.
	if rec := get("/internal/users/by-email/nobody@example.com", "Bearer "+token); rec.Code != http.StatusNotFound {
		t.Errorf("unknown email status = %d, want %d, body = %s", rec.Code, http.StatusNotFound, rec.Body.String())
	}
}
