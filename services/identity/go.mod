module github.com/TiagoJVO/beekeepingit/services/identity

go 1.25.0

// The sibling library modules are resolved from the working tree (they carry
// code this service depends on that isn't published yet — e.g. shared/devseed,
// servicetemplate/authn's org-resolve middleware + authtest). go.work already
// does this for local builds; these replaces additionally keep `go mod tidy`
// and the Docker build (which don't read go.work) resolving locally.
