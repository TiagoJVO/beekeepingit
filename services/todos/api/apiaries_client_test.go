...
func TestApiaryVerifier_BelongsToOrg_200IsTrue(t *testing.T) {
	...
	// auth.md §4) — todos never asserts org membership to apiaries out of
...
// case this file exists for (#51's carry-over from #38's review, mirroring
// #284's "fix(apiaries): close cross-tenant IDOR on counter sync"): apiaries'
// own GET /v1/apiaries/{id} 404s for an id that doesn't exist OR belongs to a
// different organization (ADR-0002 scope-hiding) — both must be treated as
