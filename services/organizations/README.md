...
| `GET /v1/organizations/me`                                    | Keycloak JWT | The caller's own organization, resolved from their active membership. Before a `404`, auto-accepts a pending invitation matching the caller's own JWT-verified, verified-flag-gated email (FR-ONB-3) if one exists — the signal the client's org-completion gate probes for. |
