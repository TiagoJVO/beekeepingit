...
// maxRegistrationNumberLength bounds the per-apiary beekeeper
...
const maxRegistrationNumberLength = 50
...
type apiaryCreateRequest struct {
	...
	// RegistrationNumber (FR-AP-9, #296) is the per-apiary OVERRIDE of
	// the organization's beekeeper registration-number default -- only set
	// when one organization covers several beekeepers. Absent/empty means
	// the apiary inherits the organization's value.
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err := withTx(r.Context(), pool, func(q *sqlcgen.Queries) error {
			...
			row, err = q.InsertApiaryWithLocation(r.Context(), sqlcgen.InsertApiaryWithLocationParams{
				...
				RegistrationNumber: notesParam(body.RegistrationNumber),
			...
			want := restRowState{name: row.Name, hive: hiveCount, notes: textOf(row.Notes), placeLabel: textOf(row.PlaceLabel), registrationNumber: textOf(row.RegistrationNumber)}
...
func respondIdempotentCreateOrConflict(w http.ResponseWriter, r *http.Request, q *sqlcgen.Queries, org pgtype.UUID, id uuid.UUID, body apiaryCreateRequest, hiveCount int32) {
	...
	sameRegistrationNumber := textOf(existing.RegistrationNumber) == strPtrValue(body.RegistrationNumber)
	if existing.Name != body.Name || existing.HiveCount != hiveCount || !sameLocation || !sameNotes || !samePlaceLabel || !sameRegistrationNumber {
	...
	writeJSON(w, r, http.StatusCreated, apiaryDTO{
		...
		RegistrationNumber: textPtr(existing.RegistrationNumber),
...
func validateCreate(body apiaryCreateRequest) (uuid.UUID, []problem.FieldError) {
	...
	if body.RegistrationNumber != nil && len(*body.RegistrationNumber) > maxRegistrationNumberLength {
		errs = append(errs, problem.FieldError{Field: "registration_number", Code: "too_long", Message: "registration_number must be at most 50 characters"})
	}
...
func updateApiary(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		err = withTx(r.Context(), pool, func(q *sqlcgen.Queries) error {
			...
			before := restRowState{name: current.Name, hive: current.HiveCount, location: current.LocationGeojson, notes: textOf(current.Notes), placeLabel: textOf(current.PlaceLabel), registrationNumber: textOf(current.RegistrationNumber)}
			want = before
			if nameSet {
				want.name = *body.Name
			}
			...
			if registrationNumberSet {
				want.registrationNumber = strPtrValue(body.RegistrationNumber)
			}
		...
		writeJSON(w, r, http.StatusOK, apiaryDTO{
			...
			RegistrationNumber: textPtr(updated.RegistrationNumber),
...
func validateUpdate(fields map[string]json.RawMessage, body apiaryUpdateRequest, nameSet bool) []problem.FieldError {
	...
	_, placeLabelSet := fields["place_label"]
	_, registrationNumberSet := fields["registration_number"]
	if !nameSet && !hiveSet && !locSet && !notesSet && !placeLabelSet && !registrationNumberSet {
...
type restRowState struct {
	...
	// override, inherit the organization's default". A business
	// registration identifier the beekeeper must display at the apiary in
	// any case, so not personal data (§7.3).
