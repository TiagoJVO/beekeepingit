...
| `validation/sync-ops.validation.json`     | Not an OpenAPI document: the **shared validation description** — the single definition of the mechanical rules a sync write-back op must satisfy, mirrored by the offline client's pre-push parity check and bound to each owning service's own constants by its package tests. See [`validation/README.md`](validation/README.md) and [ADR-0025](../docs/adr/0025-sync-validation-parity-description.md). |
