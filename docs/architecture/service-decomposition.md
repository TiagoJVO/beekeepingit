...
| 3   | **apiaries** (`apiaries`)           | Apiary CRUD; **hive count** (D-2, as one of the typed **apiary_counters**, D-20); **geo** (PostGIS) for proximity ordering & distance; search; **stock declarations** (FR-AP-10 — point-in-time records keyed by beekeeper registration number, distinct from the live counter). | `apiaries` (incl. `location geography(Point)`, `registration_number` override — FR-AP-9), `apiary_counters` (1-N, e.g. `hive`), `stock_declarations` | FR-AP-1..10                                           |
