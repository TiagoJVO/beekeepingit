...
| **Create account**            | **Built (#647, §8.11):** the login screen's own **"Create account"** action (secondary; "Sign in" keeps the one honey primary) starts the **same** authorize request as sign-in and enters the enrolment flow at **`OIDC_REGISTRATION_URL`** = `https://auth.beekeepingit.local:8443/if/flow/beekeepingit-enrollment/` — a config value like `OIDC_ACCOUNT_URL`, so the flow slug stays a deployment detail (D-7) — carrying the pending request in an origin-relative `next`. Replaces #366's instructional copy ("New here? Tap Sign in…"). No new redirect URI, no change to the token contract.                                                                                                                                                                                                                                                                                                                                                                 |
...
- **Every send claims its attempt before the mail leaves (#854).** Both send paths take the
  ...
  status line, never a free attempt after mail has already gone out (the claim also resets
  `delivery_status` to `pending`, so a row whose outcome never landed reads _unknown_, not
  the previous attempt's result).
- **The budget counts messages, not invitations.** `CountInvitationDeliveryBudgetSince`
  sums `delivery_attempts` over the rows created or last attempted inside the window. A
  per-row count — the first shape of the #854 fix, caught in its security review — charged
  an invitation once however much mail it emitted, so a handful of invitations resent on a
  loop ran about ten times past the stated ceiling. The sum's one approximation is in the
  safe direction: resending an invitation created _before_ the window drags its whole
  attempt history into the sum, so it can cost several slots for one message. An exact
  per-attempt ledger needs a table, and a new table in this schema joins the PowerSync
  publication, so it is deliberately left as follow-up.
...
- **Enrollment flow, config-as-code.** The blueprint
  ...
  **same policy-gated `user_write` stamp** (`attributes.email_verified: true` only on
  ...
  OIDC redirect finishes **back in the app**, straight into onboarding.
  - **The app now enters that chain directly (#647, FR-ONB-1/FR-UX-1).** As #366 shipped it, the
    client kept a single sign-in action and pointed new users at the provider's link with
    instructional copy — a new user was told to press the button labelled the opposite of what
    they wanted. The login screen now carries its own **"Create account"** action (secondary
    emphasis; "Sign in" keeps the screen's one honey primary), and that copy is gone. It changes
    **no provider state and no chain**: the action builds the **same** authorize request as
    "Sign in" and puts the browser on `/if/flow/beekeepingit-enrollment/?next=<that authorize
path, relative>` — the same URL `reverse_with_qs` produces for the "Sign up" link, because
    `FlowExecutorView.dispatch` stores `?next` for **whichever** flow it is serving. The entry
    point is client **config** (`OIDC_REGISTRATION_URL`), so the slug stays a deployment detail
    (D-7; oidc-integration.md §7). The provider's own link is unchanged and still works — it is
    the fall-back for a deployment that configures no entry point.
  - **`next` is now written by the client, so it is pinned live, not argued from upstream
    source.** Everywhere else in this document the `?next` chain is server-written: the
    unauthenticated authorize view puts it there (§8.13). This is the first path where the app
    composes it, so `client/e2e/tests/registration.spec.ts` was moved onto the new button and
    asserts, against the running stack, that the URL reaches
    `/if/flow/beekeepingit-enrollment/` carrying a relative `next` whose authorize request still
    has `response_type=code`, `code_challenge_method=S256`, a `code_challenge` and a `state` —
    and the rest of that spec then proves enrolment still finishes back in the app. Treat a
    version bump that breaks it as the signal, rather than re-reading upstream (the
    oidc-integration.md §8 watch-list).
  - **One ordering consequence, deliberate.** The browser no longer visits
    `/application/o/authorize/` **first**, so `check_redirect_uri` and the `client_id` check now
    run at the **end** of the chain rather than before enrolment. No check is removed or
    weakened — they still run when the browser lands on the authorize URL — but a deployment
    with a misconfigured client would strand a user who has already created an account, instead
    of failing before they started.
