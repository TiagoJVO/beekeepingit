...
| **Platform** (§5.3.2)     | membership of the IdP **`platform-operator`** group, as a verified claim (§3.3) | **every** organization    | **Built for the five existing organization-scoped routes** (#466, [ADR-0021](../adr/0021-platform-operator-tenancy-carve-out.md)) **and for `GET /organizations`, the list-all-organizations endpoint** (#467); cross-org membership lookup **planned** — EPIC-18 ([#463](https://github.com/TiagoJVO/beekeepingit/issues/463), [#468](https://github.com/TiagoJVO/beekeepingit/issues/468)) |
...
#### 5.3.2 Platform tier (EPIC-18 #463 — authority minted, existing routes carved out, and the first new endpoint built; more still to come)
...
> only — no member roster, no invitation data, nothing from inside any organization. **Not yet
> built:** cross-org membership lookup ([#468](https://github.com/TiagoJVO/beekeepingit/issues/468))
> and the persisted, distinguishable history record
> ([#470](https://github.com/TiagoJVO/beekeepingit/issues/470)). The rest of this section records
> the **intended** model ([D-32](../../requirements/decisions.md)) so the org tier above is not read
> as the whole story.
...
| Scope               | **Built:** the five existing organization-scoped routes (get/update organization, list/remove members, change role) — carved out per endpoint, [#466](https://github.com/TiagoJVO/beekeepingit/issues/466)/[ADR-0021](../adr/0021-platform-operator-tenancy-carve-out.md) — plus `GET /organizations`, the list-all-organizations endpoint ([#467](https://github.com/TiagoJVO/beekeepingit/issues/467)), which reuses the same `isPlatformOperator` claim check but rejects a non-operator with `403` rather than `404` (no `{orgId}` to hide). **Planned:** cross-org membership lookup ([#468](https://github.com/TiagoJVO/beekeepingit/issues/468)) |
