...
#### 5.3.2 Platform tier (EPIC-18 #463 — authority minted, five existing routes carved out, and the first NEW cross-org endpoint built)
...
> regression tests run unmodified). **Built:** the cross-org membership lookup
> ([#468](https://github.com/TiagoJVO/beekeepingit/issues/468)) —
> `GET /organizations/platform/memberships?email=|user_id=` — the first genuinely NEW
> platform-tier endpoint, calling `isPlatformOperator(r)` directly and returning `403` (not `404`:
> there is no `{orgId}` in this path to hide) for every other caller. **Not yet built:** list
> organizations ([#467](https://github.com/TiagoJVO/beekeepingit/issues/467)) and the persisted,
...
| Scope               | **Built:** the five existing organization-scoped routes (get/update organization, list/remove members, change role) — carved out per endpoint, [#466](https://github.com/TiagoJVO/beekeepingit/issues/466)/[ADR-0021](../adr/0021-platform-operator-tenancy-carve-out.md) — plus the new cross-org membership lookup, `GET /organizations/platform/memberships` ([#468](https://github.com/TiagoJVO/beekeepingit/issues/468)), reusing the same claim check via `isPlatformOperator(r)`. **Planned:** list-all-organizations ([#467](https://github.com/TiagoJVO/beekeepingit/issues/467)) |
