...
- **IdP groups/roles** are kept minimal: every end user is simply an **authenticated user**. A
  **`platform-operator`** — an Authentik **group** (not a realm role, not an app role) — exists for
  **operations/superadmin** (managing the IdP/infra). It is still **not** an application role: it
  never participates in the org-scoped `admin`/`user` decision below, and the **PWA**'s authZ path
  never reads it. **Since #465 (EPIC-18) it is also the source of the PLATFORM tier** — the app
  owner who administers _every_ organization — surfaced as the verified `platform_operator` claim
  on **admin-app tokens only**
  ([oidc-integration.md §3.2](oidc-integration.md#32-platform-operator-claim-platform_operator-465--epic-18-463)).
  The claim is minted from real IdP group membership and can neither be requested nor injected by a
  client; the services-side authorization that consumes it is
...
| `preferred_username`, `name`, `groups`         | profile / i18n (EN-PT, NFR-I18N); `groups` is **informational only** — it is emitted on **both** clients, so services must **never** authorize on it (§3.3)                                                                                                   |
| `platform_operator`                            | **Admin-client tokens only** (#465): verified `platform-operator` membership as a boolean — the platform tier's authority ([oidc-integration.md §3.2](oidc-integration.md#32-platform-operator-claim-platform_operator-465--epic-18-463)). Absent ⇒ **false** |
...
admin** in the org tier — a platform super-admin is the **`platform-operator`** group (§3.3), not a
membership role; NFR-ROL-1's "more roles later" can add one when needed. **EPIC-18
([#463](https://github.com/TiagoJVO/beekeepingit/issues/463)) builds that platform tier _beside_ this
table, not inside it**: the org-scoped `admin`/`user` semantics above are unchanged, and platform
authority is carried by the separate, IdP-verified `platform_operator` claim (§3.3, minted by #465,
enforced services-side by [#466](https://github.com/TiagoJVO/beekeepingit/issues/466)). _This resolves
