...
> **Migration in progress (D-7 → Authentik, [ADR-0016](../adr/0016-replace-keycloak-with-authentik.md)).**
> #24 originally hardened this slice **against Keycloak** (a realm import, an RP-initiated
> refresh-token-POST logout). The provider swap re-lands the same guarantees on **Authentik**: the
> **infra + backend rows below already reflect the Authentik reality** (WS-A blueprint, WS-B
> `oidc_sub` rename — both merged); the **client rows describe the #24 Keycloak baseline still being
> migrated** to the front-channel `end_session` logout of §7 (the client swap lands with the
> migration's client workstream). The frozen values are in
> [oidc-integration.md](oidc-integration.md).

| §7/§3.3 item                          | Where it landed                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Logout — server-side SSO revoke (NFR-SEC-1)** | [`client/lib/core/auth/auth_controller.dart`](../../client/lib/core/auth/auth_controller.dart) `logout()` revokes the **server-side SSO session**, not just local tokens, and degrades to local-only clearing offline (D-10). **#24 baseline:** a refresh-token **POST** to Keycloak's logout endpoint. **Target (§7):** a **front-channel `end_session` GET** with `id_token_hint` under Authentik — the client migration re-points `AppConfig` off the Keycloak `/protocol/openid-connect/logout` path.                                       |
