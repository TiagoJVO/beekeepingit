...
- **Redirect URIs** — `http://localhost:5175` (**strict**), `http://localhost:5174` (**strict**), `https://app\.beekeepingit\.local:8443/.*` (regex), **and** `https://app.beekeepingit.local:8443` (**strict**). The strict bare-origin entry is required because the PWA sends the bare origin as its `redirect_uri`, and Authentik derives **CORS**-allowed origins from `redirect_uris` — an `Origin` has no path, so the `…/.*` regex never matches it.

  The localhost dev origins are **strict literals, one per real dev port** — no regex — per the owner decision on [#822](https://github.com/TiagoJVO/beekeepingit/issues/822). Matching is `re.fullmatch`, so the original `http://localhost:.*` also accepted `http://localhost:@evil.example`, a URL the browser resolves to host `evil.example` with `localhost:` as **userinfo**, leaking the `?code=` to an attacker origin that could redeem it (no client secret on a public client, PKCE not mandatory). Both narrower regexes proposed to replace it are unsafe on their own axis:

  - `http://localhost:\d+(/.*)?` — Python `\d` matches **Unicode** decimal digits, so `http://localhost:٤٥` fullmatches and is allow-listed; and `urlparse` of it yields hostname `localhost` and then **raises** on `.port`, reproducing the `.*` bug where any localhost `Origin` **500s** `/token` and `/userinfo` through `cors_allow`.
  - `http://localhost:[0-9]+(/.*)?` — a **bracketed host**. `urlparse` treats a bracketed netloc as an IPv6 literal: on Python 3.11.0 the hostname is `0-9` (so it grants no localhost CORS at all), and later CPython raises `ValueError: Invalid IPv6 URL`, which 500s the discovery document for any request carrying an `Origin` header.

  A strict literal has no regex metacharacter, so it parses identically on every Python version, has no Unicode-digit surface, and `urlparse` returns a real hostname and port — which is also what finally grants a localhost dev origin CORS on `/token` and `/userinfo`. `5175` is the Flutter client's dev server (`flutter run -d chrome` binds a random ephemeral port, so `client/README.md` pins `--web-port 5175`); `5174` is the admin app's Vite dev server (`admin/vite.config.ts`), present on **this** provider for the same reason as the strict `adminOrigin` entry — the admin app reads this application's discovery + JWKS cross-origin (#460). Adding a dev port is a deliberate edit in both the blueprint and the guard.

  Held by `scripts/check-authorization-redirect-posture.sh` (`task repo:lint`), which allows only those forms plus origins rendered from `global.appOrigin`/`global.adminOrigin`, and **bans `[`, `]` and `\d` in any redirect URI** — on **every** `redirect_uri_type`, because `token.py` derives the /token + /userinfo CORS origins from **all** `redirect_uris`, unfiltered.
...
- **Redirect URIs** — `http://localhost:5174` (**strict**, the Vite dev server pinned by
  `admin/vite.config.ts`; a strict literal per real dev port since #822, same
